...
/**
 * Base class of Model\DeepEmbeddedDocument document.
 */
abstract class DeepEmbeddedDocument extends \Mandango\Document\EmbeddedDocument
{
    /**
     ...
     *
     * @return Model\DeepEmbeddedDocument The document (fluent interface).
     */
    public function setDocumentData($data, $clean = false)
    {
        ...
        if (isset($data['embedded_one'])) {
            ...
            if ($rap = $this->getRootAndPath()) {
                $embedded->setRootAndPath($rap['root'], $rap['path'].'.embedded_one');
            }
    ...
    public function getEmbeddedOne()
    {
        if (!isset($this->data['embeddeds_one']['embedded_one'])) {
            if (
                (!isset($this->data['embeddeds_one']) || !array_key_exists('embedded_one', $this->data['embeddeds_one']))
                &&
                ($rap = $this->getRootAndPath())
                &&
                !$this->isEmbeddedOneChangedInParent()
